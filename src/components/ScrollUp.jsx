...
import { useEffect, useState } from "react"; // ✅ Убрали лишний import React
...
const ScrollUp = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      isScrollState(window.scrollY >= 450);
    };

    window.addEventListener("scroll", handleScroll);

    // ✅ Обязательно: убираем слушатель при размонтировании + пустой массив зависимостей
    return () => window.removeEventListener("scroll", handleScroll);
  }, []); // ✅ Пустой массив = эффект сработает только при маунте

  return (
    <button
      type="button"
      aria-label="Прокрутить вверх" // ✅ Доступность для скринридеров
      className={`${
        isScroll ? "bottom-24 xl:bottom-16" : "-bottom-1/2"
      } w-10 h-10 bg-light dark:bg-[#272727] shadow-md flex justify-center items-center rounded z-10 fixed right-4 xl:right-8 hover:-translate-y-2 duration-300 btnHover overflow-hidden`}
